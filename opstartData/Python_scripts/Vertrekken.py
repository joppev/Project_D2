...
def ocr(IMAGE_PATH):
    SECRET_KEY = 'sk_7a9af2a2848a1350b09450ad'
...
try:
    while True:
        ...
        if uitkomstdef is None:
            ...
            print (uitkomstdef)
            sql_select_Query = "SELECT p.* FROM plannings p JOIN users u ON u.id = p.gebruikerID JOIN nummerplaats n ON u.bedrijfsID = n.bedrijfID WHERE n.plaatcombinatieZonderStreepjes = %s AND p.startTijd >= %s AND p.startTijd <= %s AND p.isAanwezig = True ORDER BY p.startTijd ASC LIMIT 1"
            ...
            if mycursor.rowcount == 1:
                sql_select_update = "UPDATE plannings SET isAanwezig = False WHERE id = %s"
